...
from snake_case_migration.audit import (
    audit_python_source,
...
""",
    ...
    assert any(
        ".def" in message and "getAngularPositionRotations" in message
        for message in messages
    )
    assert any(
        ".def_readwrite" in message and "pixelFormat" in message for message in messages
    )
    assert any(
        ".def_static" in message and "enumerateDevices" in message
        for message in messages
    )
    assert any(
        "rename" in message and "angularPosition" in message for message in messages
    )
    assert any(
        "getHTTPServer" in message and "expected 'get_http_server'" in message
        for message in messages
    )
